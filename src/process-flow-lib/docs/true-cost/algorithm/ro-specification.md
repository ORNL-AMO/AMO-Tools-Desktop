...
This document specifies cost attribution for diagrams that contain a **Reverse Osmosis (RO) configuration**. It is an extension of [algorithm-v4.md](algorithm-v4.md) and defines **Section 5** of the Cost Attribution framework — read that document first. The rule here, **ro-reject-redirect**, overrides the standard proportional rules (no-losses-path, with-losses-path, mid-chain-fork, reuse-and-discharge-split, and the discharge branch-ratio product rule's proportional-discharge, merge-node-loss, and unaccounted-flow cases) wherever an RO configuration is detected.
