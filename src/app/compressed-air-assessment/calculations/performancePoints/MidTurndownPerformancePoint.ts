...
export class MidTurndownPerformancePoint implements PerformancePoint {
    ...
    setAirflow(compressorControls: CompressorControls, fullLoad: FullLoadPerformancePoint, settings: Settings) {
        if (this.isDefaultAirFlow) {
            let defaultAirflow: number = ((1 - (compressorControls.unloadPointCapacity / 100)) / 2 + (compressorControls.unloadPointCapacity / 100)) * fullLoad.airflow;
