...
export class CompressedAirCostSavingsGraphComponent {
  ...
  drawDayTypeModificationGraph() {
    if (this.assessmentResults && this.combinedDayTypeResults && this.combinedDayTypeResults.length != 0 && this.modificationGraph) {
      ...
      this.assessmentResults.forEach(result => {
        ...
        xValue.push(dayTypeResult.modificationSavings.adjustedResults.cost);
      });
      xValue.unshift(baselineDayTypeResult.cost);
      let traceData = [];
  ...
  drawCombinedDayTypeModificationGraph() {
    if (this.assessmentResults && this.combinedDayTypeResults && this.combinedDayTypeResults.length != 0 && this.modificationGraph) {
      const chart = this.compressedAirChartsService.buildCostSavingsChart(this.assessmentResults, this.combinedDayTypeResults);
      var config = {
        responsive: true,
        displaylogo: false
      };
  ...
  getTrace(xValue: Array<number>, y: Array<string>, name: string, text?: Array<string>) {
    return {
      ...
      orientation: 'h',
      name: name,
      hovertemplate: name + ': $%{x:,.0f}<extra></extra>',
      text: text,
      textposition: 'auto',
      marker: {
        line: {
