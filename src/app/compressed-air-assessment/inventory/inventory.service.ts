...
import { CentrifugalSpecifics, CompressedAirAssessment, CompressedAirDayType, CompressorControls, CompressorInventoryItem, CompressorNameplateData, DesignDetails, Modification, PerformancePoint, PerformancePoints, ProfileSummary, ProfileSummaryData, ReduceRuntimeData, SystemInformation, SystemProfileSetup } from '../../shared/models/compressed-air-assessment';
...
export class InventoryService {
  ...
  addNewCompressor(compressedAirAssessment: CompressedAirAssessment, newInventoryItem?: CompressorInventoryItem, selectedModificationId?: string): {newInventoryItem: CompressorInventoryItem, compressedAirAssessment: CompressedAirAssessment} {
    if (!newInventoryItem) {
      newInventoryItem = this.getNewInventoryItem();
    }

    newInventoryItem.modifiedDate = new Date();
    if (selectedModificationId !== undefined) {
      newInventoryItem.isNewModified = true;
      let selectedModificationIndex = compressedAirAssessment.modifications.findIndex(mod => { return mod.modificationId == selectedModificationId});
      compressedAirAssessment.modifications[selectedModificationIndex].modifiedCompressorInventoryItems.push(newInventoryItem);
    } else {
