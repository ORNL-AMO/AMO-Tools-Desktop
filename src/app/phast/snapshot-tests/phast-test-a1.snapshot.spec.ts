/**
 * Snapshot test: TEST A1
 *
 * Fuel-fired, Imperial, 12 wall losses, 2 cooling losses, 2 modifications.
 *
 * To regenerate snapshots:
 *   1. Set GENERATE = true
 *   2. Run: ng test --include="**\/phast-test-a1.snapshot.spec.ts" --watch=false --browsers=ChromeHeadless
 *   3. Pipe output through generate-snapshot.py into snapshots/test-a1.snap.json
...
const fixture = require('./fixtures/TEST A1.json');

const GENERATE = false;

const MODIFICATION_COUNT = 2;
...
describe('Snapshot Test: TEST A1', () => {
  ...
  if (GENERATE) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const snapshots = require('./snapshots/test-a1.snap.json');
