/**
 * Snapshot test: TEST B1
 *
 * Fuel-fired, Imperial, 1 charge material, 1 wall loss, 1 cooling loss, 1 modification.
 *
 * To regenerate snapshots:
 *   1. Set GENERATE = true
 *   2. Run: ng test --include="**\/phast-test-b1.snapshot.spec.ts" --watch=false --browsers=ChromeHeadless
 *   3. Pipe output through generate-snapshot.py into snapshots/test-b1.snap.json
...
const fixture = require('./fixtures/TEST B1.json');
...
describe('Snapshot Test: TEST B1', () => {
  ...
  if (GENERATE) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const snapshots = require('./snapshots/test-b1.snap.json');
