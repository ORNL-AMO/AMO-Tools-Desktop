...
@Component({
  selector: 'app-ssmt-rollup-steam-summary',
  templateUrl: './ssmt-rollup-steam-summary.component.html',
  styleUrls: ['./ssmt-rollup-steam-summary.component.css']
})
export class SsmtRollupSteamSummaryComponent implements OnInit {
  @Input()
  settings: Settings;
  @Input()
  printView: boolean;
  ...
  ngOnInit() {
    ...
    this.resultsSub = this.reportRollupService.ssmtResults.subscribe((ssmts: Array<SsmtResultsData>) => {
      if (ssmts.length !== 0) {
        this.numSsmts = ssmts.length;
        this.resultData = ssmts;
        if (this.printView) {
