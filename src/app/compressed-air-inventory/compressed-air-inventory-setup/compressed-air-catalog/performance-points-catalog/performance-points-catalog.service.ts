...
export class PerformancePointsCatalogService {
  ...
  checkPerformancePointsValid(compressor: CompressedAirItem, systemInformation: SystemInformation): boolean {
    ...
    let showMidTurndown: boolean = this.checkShowMidTurndown(compressor.compressedAirControlsProperties.controlType);
    if (isValid && showMidTurndown) {
      let midTurndownForm: UntypedFormGroup = this.getPerformancePointFormFromObj(compressor.compressedAirPerformancePointsProperties.midTurndown, compressor, 'midTurndown', systemInformation);
      isValid = midTurndownForm.valid;
    }

    let showTurndown: boolean = this.checkShowTurndown(compressor.compressedAirControlsProperties.controlType);
    if (isValid && showTurndown) {
      let turndownForm: UntypedFormGroup = this.getPerformancePointFormFromObj(compressor.compressedAirPerformancePointsProperties.turndown, compressor, 'turndown', systemInformation);
      isValid = turndownForm.valid;
    } 
