  ...
  setAllDbData() {
      ...
      .pipe(catchError(error => this.appErrorService.handleObservableAppError('Error loading MEASUR database', error))).subscribe({
        next: (initializedData) => {
          ...
          // if (this.electronService.isElectron) {
          //   // todo 6925 needs to go back to being subscribed to
          //   let applicationInstanceData: ApplicationInstanceData = this.applicationInstanceDbService.applicationInstanceData.getValue();
          //   if (applicationInstanceData && !this.automaticBackupService.observableDataChanges) {
          //     console.log('setAllDbData')
          //     this.automaticBackupService.initializeBackups(applicationInstanceData);
          //   }
          // }
