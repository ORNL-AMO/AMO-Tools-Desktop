...
import {
  ...
  PowerFactorCorrectionTreasureHunt, FanAffinityLawTreasureHunt
...
export class SortCardsService {
  ...
  constructor(private opportunityCardsService: OpportunityCardsService,
    ...
    private fanAffinityLawTreasureHuntService: FanAffinityLawTreasureHuntService
    ) { }
  ...
  sortTreasureHunt(treasureHunt: TreasureHunt, sortBy: SortCardsData, settings: Settings): TreasureHunt {
    ...
    let hasCoolingTowerBasin: boolean = calculatorTypes.includes(Treasure.coolingTowerBasin);
    ...
    let hasFanAffinityLaw: boolean = calculatorTypes.includes(Treasure.fanAffinityLaw);
    ...
    if (allCalcTypes || hasMakeupWater) {
    ...
    let chillerStagingOpportunities: Array<ChillerStagingTreasureHunt> = [];
    if (allCalcTypes || hasChillerStaging) {
      if (treasureHunt.chillerStagingOpportunities && treasureHunt.chillerStagingOpportunities.length !=0) {
        chillerStagingOpportunities = this.sortChillerStagingOpportunities(treasureHunt.chillerStagingOpportunities, sortBy, treasureHunt, settings);
      }
    }
    let chillerPerformanceOpportunities: Array<ChillerPerformanceTreasureHunt> = [];
    if (allCalcTypes || hasChillerPerformance) {
      if (treasureHunt.chillerPerformanceOpportunities && treasureHunt.chillerPerformanceOpportunities.length !=0) {
    ...
    let coolingTowerBasinOpportunities: Array<CoolingTowerBasinTreasureHunt> = [];
    if (allCalcTypes || hasCoolingTowerBasin) {
      if (treasureHunt.coolingTowerBasinOpportunities && treasureHunt.coolingTowerBasinOpportunities.length !=0) {
        coolingTowerBasinOpportunities = this.sortCoolingTowerBasinOpportunities(treasureHunt.coolingTowerBasinOpportunities, sortBy, treasureHunt, settings);
      }
    }
    let boilerBlowdownRateOpportunities: Array<BoilerBlowdownRateTreasureHunt> = [];
    if (allCalcTypes || hasBoilerBlowdownRate) {
      if (treasureHunt.boilerBlowdownRateOpportunities && treasureHunt.boilerBlowdownRateOpportunities.length !=0) {
        boilerBlowdownRateOpportunities = this.sortBoilerBlowdownRateOpportunities(treasureHunt.boilerBlowdownRateOpportunities, sortBy, treasureHunt, settings);
      }
    }
    let powerFactorCorrectionOpportunities: Array<PowerFactorCorrectionTreasureHunt> = [];
    if (allCalcTypes || hasPowerFactorCorrection) {
      if (treasureHunt.powerFactorCorrectionOpportunities && treasureHunt.powerFactorCorrectionOpportunities.length != 0) {
    ...
    let fanAffinityLawOpportunities: Array<FanAffinityLawTreasureHunt> = [];
    if (allCalcTypes || hasFanAffinityLaw) {
      if (treasureHunt.fanAffinityLawOpportunities && treasureHunt.fanAffinityLawOpportunities.length != 0) {
        fanAffinityLawOpportunities = this.sortFanAffinityLawOpportunities(treasureHunt.fanAffinityLawOpportunities, sortBy, treasureHunt, settings);
      }
    }
    ...
    let filteredTreasureHunt: TreasureHunt = {
      ...
      fanAffinityLawOpportunities: fanAffinityLawOpportunities,
...
sortAirHeatingOpportunities(items: Array<AirHeatingTreasureHunt>, sortBy: SortCardsData, treasureHunt: TreasureHunt, settings: Settings): Array<AirHeatingTreasureHunt> {
  ...
  sortChillerStagingOpportunities(items: Array<ChillerStagingTreasureHunt>, sortBy: SortCardsData, treasureHunt: TreasureHunt, settings: Settings): Array<ChillerStagingTreasureHunt> {
    return items.filter(item => {
      let opportunitySummary: OpportunitySummary = this.opportunitySummaryService.getIndividualOpportunitySummary(item, settings);
      let cardItem: OpportunityCardData = this.chillerStagingTreasureHuntService.getChillerStagingCardData(item, opportunitySummary, 0, treasureHunt.currentEnergyUsage, settings);
      return this.checkCardItemIncluded(cardItem, sortBy);
    });
  }

  sortChillerPerformanceOpportunities(items: Array<ChillerPerformanceTreasureHunt>, sortBy: SortCardsData, treasureHunt: TreasureHunt, settings: Settings): Array<ChillerPerformanceTreasureHunt> {
    return items.filter(item => {
      let opportunitySummary: OpportunitySummary = this.opportunitySummaryService.getIndividualOpportunitySummary(item, settings);
  ...
  sortCoolingTowerFanOpportunities(items: Array<CoolingTowerFanTreasureHunt>, sortBy: SortCardsData, treasureHunt: TreasureHunt, settings: Settings): Array<CoolingTowerFanTreasureHunt> {
    return items.filter(item => {
      let opportunitySummary: OpportunitySummary = this.opportunitySummaryService.getIndividualOpportunitySummary(item, settings);
      let cardItem: OpportunityCardData = this.coolingTowerFanTreasureHuntService.getCoolingTowerFanCardData(item, opportunitySummary, 0, treasureHunt.currentEnergyUsage, settings);
      return this.checkCardItemIncluded(cardItem, sortBy);
    });
  }

  sortCoolingTowerBasinOpportunities(items: Array<CoolingTowerBasinTreasureHunt>, sortBy: SortCardsData, treasureHunt: TreasureHunt, settings: Settings): Array<CoolingTowerBasinTreasureHunt> {
    return items.filter(item => {
      let opportunitySummary: OpportunitySummary = this.opportunitySummaryService.getIndividualOpportunitySummary(item, settings);
      let cardItem: OpportunityCardData = this.coolingTowerBasinTreasureHuntService.getCoolingTowerBasinCardData(item, opportunitySummary, 0, treasureHunt.currentEnergyUsage, settings);
      return this.checkCardItemIncluded(cardItem, sortBy);
    });
  }

  sortBoilerBlowdownRateOpportunities(items: Array<BoilerBlowdownRateTreasureHunt>, sortBy: SortCardsData, treasureHunt: TreasureHunt, settings: Settings): Array<BoilerBlowdownRateTreasureHunt> {
    return items.filter(item => {
      let opportunitySummary: OpportunitySummary = this.opportunitySummaryService.getIndividualOpportunitySummary(item, settings);
      let cardItem: OpportunityCardData = this.boilerBlowdownRateTreasureHuntService.getBoilerBlowdownRateCardData(item, opportunitySummary, settings, 0, treasureHunt.currentEnergyUsage);
      return this.checkCardItemIncluded(cardItem, sortBy);
    });
  }

  sortPowerFactorCorrectionOpportunities(items: Array<PowerFactorCorrectionTreasureHunt>, sortBy: SortCardsData, treasureHunt: TreasureHunt, settings: Settings): Array<PowerFactorCorrectionTreasureHunt>{
    return items.filter(item => {
      let opportunitySummary: OpportunitySummary = this.opportunitySummaryService.getIndividualOpportunitySummary(item, settings);
      let cardItem: OpportunityCardData = this.powerFactorCorrectionTreasureHuntService.getPowerFactorCorrectionCardData(item, opportunitySummary, 0, treasureHunt.currentEnergyUsage, settings);
      return this.checkCardItemIncluded(cardItem, sortBy);
    });
  }

  sortFanAffinityLawOpportunities(items: Array<FanAffinityLawTreasureHunt>, sortBy: SortCardsData, treasureHunt: TreasureHunt, settings: Settings): Array<FanAffinityLawTreasureHunt>{
    return items.filter(item => {
      let opportunitySummary: OpportunitySummary = this.opportunitySummaryService.getIndividualOpportunitySummary(item, settings);
      let cardItem: OpportunityCardData = this.fanAffinityLawTreasureHuntService.getFanAffinityLawCardData(item, opportunitySummary, 0, treasureHunt.currentEnergyUsage, settings);
      return this.checkCardItemIncluded(cardItem, sortBy);
    });
  }
}
