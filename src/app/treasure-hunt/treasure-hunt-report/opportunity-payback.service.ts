...
export class OpportunityPaybackService {
  ...
  getOpportunityPaybackDetails(opportunitySummaries: Array<OpportunitySummary>): OpportunitiesPaybackDetails {
    let lessThanOneYearOpps: Array<OpportunitySummary> = _.filter(opportunitySummaries, (opp: OpportunitySummary) => { return opp.payback < 1 });
    ...
    let oneToTwoYearsOpps: Array<OpportunitySummary> = _.filter(opportunitySummaries, (opp: OpportunitySummary) => { return opp.payback >= 1 && opp.payback < 2 });
    ...
    let twoToThreeYearsOpps: Array<OpportunitySummary> = _.filter(opportunitySummaries, (opp: OpportunitySummary) => { return opp.payback >= 2 && opp.payback < 3 });
    ...
    let moreThanThreeYearsOpps: Array<OpportunitySummary> = _.filter(opportunitySummaries, (opp: OpportunitySummary) => { return opp.payback > 3 });
